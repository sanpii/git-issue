...
testSyncPush () {
	...
	cd "$REPO"
	output="$(git issue list --color=none)"
...
testSyncConflict () {
	...
	cd "$REPO"
	git issue init -q
	git issue new -q --no-edit --title='test 1' 'Issue in the first repo'

	cd "$REPO-clone"
	git issue init -q
	git issue new -q --no-edit --title='test 1' 'Issue in the second repo'
